...
>
> **คุณTop owner architecture:** Rex Project เป็น Rex-only environment; Rex context ถูก preload ตั้งแต่เปิด chat. สำหรับงาน code/project ทั่วไปให้ใช้ ChatGPT Project ที่ bind เฉพาะ code Project (เช่น `s002`, `wm`) ตาม `Agent-Team/CHATGPT_BOOTSTRAP.md`; chat ใหม่เริ่มด้วย base model และค่อยใช้ `/a <staff>` เฉพาะเมื่อต้องการ Staff. ไม่ต้องติดตั้ง/คัดลอก Staff เข้า repository นี้
